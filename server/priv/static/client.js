...
// build/dev/javascript/core/tic_tac_toe.mjs
...
class Occupied extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function initial_board() {
  return toList([
    new Cell(new Square(0, 0, 8), new Empty2),
    new Cell(new Square(1, 0, 1), new Empty2),
    new Cell(new Square(2, 0, 6), new Empty2),
    new Cell(new Square(0, 1, 3), new Empty2),
    new Cell(new Square(1, 1, 5), new Empty2),
    new Cell(new Square(2, 1, 7), new Empty2),
    new Cell(new Square(0, 2, 4), new Empty2),
    new Cell(new Square(1, 2, 9), new Empty2),
    new Cell(new Square(2, 2, 2), new Empty2)
  ]);
}
function play(board, player, x, y) {
  let is_valid_move = any(board, (cell) => {
    return cell.square.x === x && cell.square.y === y && cell.state instanceof Empty2;
  });
  if (is_valid_move) {
    let new_board = map2(board, (cell) => {
      let $ = cell.square.x === x && cell.square.y === y;
      if ($) {
        return new Cell(cell.square, new Occupied(player));
      } else {
        return cell;
      }
    });
    return new Ok(new_board);
  } else {
    return new Error("Invalid move");
  }
}
function combinations_of_2(items) {
  if (items instanceof Empty) {
    return items;
  } else {
    let x = items.head;
    let rest = items.tail;
    let with_x = map2(rest, (y) => {
      return toList([x, y]);
    });
    return append(with_x, combinations_of_2(rest));
  }
}
function combinations_of_3(items) {
  if (items instanceof Empty) {
    return items;
  } else {
    let x = items.head;
    ...
    let with_x = map2(pairs, (pair) => {
      return prepend(x, pair);
    });
    return append(with_x, combinations_of_3(rest));
  }
}
function check_win(board, player) {
  let player_values = filter_map(board, (cell) => {
    let $ = cell.state;
    if ($ instanceof Occupied) {
      let p = $[0];
      if (isEqual(p, player)) {
        return new Ok(cell.square.value);
      } else {
        return new Error(undefined);
      }
    } else {
  ...
  return any(combos, (combo) => {
    return sum(combo) === 15;
  });
}
function get_game_state(board) {
  let x_wins = check_win(board, new X);
  let o_wins = check_win(board, new O);
  let is_full = all(board, (cell) => {
    let $ = cell.state;
...
class Model extends CustomType {
  constructor(board, current_player, game_state) {
    super();
    this.board = board;
...
class UserClickedRestart extends CustomType {
}
function init(_) {
  let board = initial_board();
  let model = new Model(board, new X, new InProgress);
  return [model, none()];
}
function update2(model, msg) {
  if (msg instanceof UserClickedSquare) {
    let x = msg.x;
    let y = msg.y;
    let $ = model.game_state;
    if ($ instanceof InProgress) {
      let $1 = play(model.board, model.current_player, x, y);
      if ($1 instanceof Ok) {
        let new_board = $1[0];
        let new_state = get_game_state(new_board);
        let _block;
        let $2 = model.current_player;
        if ($2 instanceof X) {
          _block = new O;
        } else {
          _block = new X;
        }
        let next_player = _block;
        return [new Model(new_board, next_player, new_state), none()];
      } else {
        return [model, none()];
      }
    } else {
    ...
    let board = initial_board();
    return [new Model(board, new X, new InProgress), none()];
  }
}
function view_cell(state, x, y) {
  let _block;
  if (state instanceof Empty2) {
    _block = "cell";
  } else {
    let $ = state[0];
    if ($ instanceof X) {
  ...
  return div(toList([
    class$(class_name),
    on_click(new UserClickedSquare(x, y))
  ]), toList([text3(content)]));
}
function view_board(board) {
  return div(toList([class$("board")]), (() => {
    let _pipe = map2(toList([0, 1, 2]), (y) => {
      return map2(toList([0, 1, 2]), (x) => {
        let cell = find(board, (c) => {
          return c.square.x === x && c.square.y === y;
        });
        if (cell instanceof Ok) {
          let c = cell[0];
          return view_cell(c.state, x, y);
        } else {
          return div(toList([class$("cell")]), toList([]));
        }
      });
    });
...
function player_to_string(player) {
...
function view_status(model) {
  let _block;
  let $1 = model.game_state;
  if ($1 instanceof InProgress) {
    _block = [
      "Player " + player_to_string(model.current_player) + "'s turn",
      "status"
    ];
  } else if ($1 instanceof Win) {
    let player = $1[0];
    _block = [player_to_string(player) + " wins!", "status win"];
  } else {
    _block = ["It's a draw!", "status"];
  }
  let $ = _block;
  let status_text;
  let class_name;
  status_text = $[0];
  class_name = $[1];
  return div(toList([class$(class_name)]), toList([text3(status_text)]));
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text3("Tic Tac Toe")])),
    view_board(model.board),
    view_status(model),
    button(toList([on_click(new UserClickedRestart)]), toList([text3("Restart")]))
  ]));
}
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "client", 16, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 376, end: 425, pattern_start: 387, pattern_end: 392 });
  }
  return;
}
